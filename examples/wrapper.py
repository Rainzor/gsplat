...
class StreamV2VWrapper:
    ...
    def _load_model(
        ...
        pipeline_cls = (
            StableDiffusionXLPipeline if self.sd_xl else StableDiffusionPipeline
        )

        try:
            # Attempt to load the model from a local directory
            pipe = pipeline_cls.from_pretrained(model_id_or_path).to(
                device=self.device, dtype=self.dtype
            )
        except ValueError:
            # If the model is not found locally, load from Hugging Face
            try:
                pipe = pipeline_cls.from_single_file(model_id_or_path).to(
                    device=self.device, dtype=self.dtype
                )
            except Exception as e:
                logging.error(f"Failed to load model from Hugging Face: {e}")
                sys.exit(
                    "Model load has failed from both local and Hugging Face sources."
                )
        except Exception as e:
        ...
        stream.prepare(
            ...
            num_inference_steps=50,
            guidance_scale=(
                1.2 if stream.cfg_type in ["full", "self", "initialize"] else 1.0
            ),
            generator=torch.manual_seed(seed),
            seed=seed,
        )
